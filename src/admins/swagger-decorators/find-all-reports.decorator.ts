...
export const ApiFindAllReports = () => {
  return applyDecorators(
    ApiBearerAuth('access-token'),
    ApiOperation({
      summary: '리포트 전체 조회(pagination) api(for admin)',
      description:
        '일단은 기획이 나온게 없고 성능 이슈 때문에 reason은 따로 response에 실어주지 않음.',
      operationId: 'admin_findAllReport',
    }),

    ApiOkResponse({
      description: '성공적으로 report 조회',
      schema: {
        properties: {
          contents: {
            $ref: getSchemaPath(ReportsPaginationResponseDto),
          },
        },
      },
    }),
    ...
    ApiExtraModels(ReportsPaginationResponseDto),
  );
};
