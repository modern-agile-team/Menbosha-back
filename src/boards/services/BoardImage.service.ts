...
export class BoardImagesService {
  ...
  async updateHelpMeBoardImages(
    ...
    const existingImages = // boardId에 해당하는 이미지url을 DB에서 불러옵니다.
      await this.boardImageRepository.getHelpMeBoardImages(boardId);
  ...
  async updateMentorBoardImages(
    ...
    await this.deleteMentorBoardImages(imagesToDelete); //이미지 삭제처리

    // 여기서부터 새로운 이미지 추가
    const newImagesArray: CreateMentorBoardImageDto[] = [];
    // 예외처리 files이 없으면 실행되지 않음.
    if (files && files.length > 0) {
      for (const file of files) {
        const uploadedImage = await this.s3Service.uploadImage(
        ...
        const boardImage = new CreateMentorBoardImageDto();
        boardImage.mentorBoardId = boardId;
        boardImage.imageUrl = uploadedImage.url;
        const savedImage =
          await this.boardImageRepository.saveMentorBoardImage(boardImage);
        newImagesArray.push(savedImage);
      }
    }
    return {
  ...
  private async deleteHelpMeBoardImages(imagesToDelete: HelpMeBoardImage[]) {
    ...
    if (s3ToDelete.length > 0) {
      await this.boardImageRepository.deleteHelpMeBoardImages(imagesToDelete); // 이미지 삭제 (db)
      await this.s3Service.deleteImage(s3ToDelete.join(',')); // 이미지 삭제 (s3)
    }
  }

  private async deleteMentorBoardImages(imagesToDelete: MentorBoardImage[]) {
    const s3ToDelete = imagesToDelete.map((image) => {
      const parts = image.imageUrl.split('/');
      const fileName = parts[parts.length - 1]; // S3에서 삭제할 이미지 파일명을 추출합니다.
      console.log(fileName);
    ...
    // 예외처리 (빈 배열이 아닐때만 삭제요청 하기)
    if (s3ToDelete.length > 0) {
      await this.boardImageRepository.deleteMentorBoardImages(imagesToDelete); // 이미지 삭제 (db)
      await this.s3Service.deleteImage(s3ToDelete.join(',')); // 이미지 삭제 (s3)
    }
  }
}
